...
import {
  CONTACT_EMAIL,
  CONTACT_PHONE_E164 as PHONE,
  SITE_NAME,
  siteOrigin,
} from '@shared/site.ts';

// Absolute site URL used for every @id and url in the graph. Defaults to the
// production origin and is overridden at build time (prerender) or on boot
// (client) so a project-site deployment emits its own URLs.
let SITE_URL = siteOrigin();

/** Point the schema graph at a different origin + base path. */
export function setSiteUrl(url: string): void {
  SITE_URL = url.replace(/\/+$/, '');
}

const logoUrl = () => `${SITE_URL}/images/tara-dealership-logo.png`;
